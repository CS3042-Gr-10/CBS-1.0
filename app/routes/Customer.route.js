...
async function indexAction(req,res){
    ...
    try{
        ...
        const accounts = await CustomerModel.getAllAccountsOfCustomer(userID);
        if (accounts.length === 0) throw new Errors.Conflict("Visit your local bank and first make a Bank Account")
